...
public abstract class BaseHttpClient {
	...
	protected static BaseHttpClient sClient;
	
	public static final String PROTOCOL_HTTP = "http";
    public static final String PROTOCOL_HTTPS = "https";
    
    public static String sDEFAULT_USER_AGENT_NAME = "Simple HTTP Client for Android";
	...
	public abstract <T> BaseResponse<T> get(BaseRequest<T> baseRequest);
	
	public abstract <T> BaseResponse<T> post(BaseRequest<T> baseRequest);
	
	protected <T> BaseResponse<T> handleNullMethodParameter(String parameterName, BaseResponse<T> response){
		response.setError(new BaseResponse.Error(BaseResponse.Error.CLIENT_EXCEPTION));
		BaseResponse.Error e = response.getError();
		e.setException(new IllegalArgumentException("baseRequest can not be null"));
		return response;
	}
	
	protected <T> BaseResponse<T> handleException(Exception e, BaseResponse<T> response){
		response.setError(new BaseResponse.Error(BaseResponse.Error.CLIENT_EXCEPTION));
	...
	protected static void setAgent(){
		sUserAgent = sCurrentUserAgentName + " by " + sCurrentAuthorName + ". "
	            + sDEVICE_BUILD_INFO;
	}
