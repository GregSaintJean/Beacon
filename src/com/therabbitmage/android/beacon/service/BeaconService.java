...
public class BeaconService extends Service implements LocationListener,
	...
	private final class ServiceHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_STOP:
				...
				BeaconApp.setBeaconStatus(false);
				break;
	...
	public void onCreate() {
		...
		mApp.setHasNetworkConnectivity(AndroidUtils
	...
	private Runnable mBeacon = new Runnable() {
		...
		public void run() {
			...
			if (!BeaconApp.isBeaconOnline() && BeaconApp.isActive()) {
				BeaconApp.setBeaconStatus(true);
			}
	...
	protected class SendMessage implements Runnable{
		...
		@Override
		public void run() {
			String message = String.format(getString(R.string.transmit_sms_beacon_message),
					mMessageToSend);
	...
	private Runnable mBeaconUpdateTwitter = new Runnable() {
		...
		public void run() {
			...
			if (BeaconApp.hasNetworkConnectivity() && mApp.hasTwitterLogin()) {
	...
	private void sendTwitterTweet(String message) {
		...
		AccessToken accessToken = new AccessToken(mApp.getTwitterAccessToken(),
				mApp.getTwitterAccessTokenSecret());
		...
		broadcast.putExtra(EXTRA_BROADCAST_MESSAGE,
				getString(R.string.tweet_sent));
		mLocalBMgr.sendBroadcast(broadcast);
		sendNotification(
